(function() {
    ...
    // Grid auto-shows on localhost for positioning, hidden in production
    var SHOW_GRID = (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1');
    ...
    function loadLocationsAndInit() {
        // Use embedded data (generated by Hugo at build time)
        if (window.LOCATION_DATA) {
            if (SHOW_GRID) createGrid();
            createPins(window.LOCATION_DATA.locations);
            initMap();
            return;
        }
        // Fallback to fetch (for backward compatibility during dev)
